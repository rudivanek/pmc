...
export const checkUserAccess = async (userId: string, userEmail: string): Promise<AccessCheckResult> => {
  try {
    ...
    console.log('📊 User subscription data:', {
      ...
      until_date: userData.until_date,
      tokens_allowed: userData.tokens_allowed
    });
    
    // Check if subscription is still valid (current date <= until_date)
    const today = new Date().toISOString().split('T')[0]; // Get today's date in YYYY-MM-DD format (UTC)
    const untilDate = userData.until_date;
    
    console.log('📅 Date comparison:', { today, untilDate });
    
    // If until_date is null or today is after until_date, deny access
    let isSubscriptionValid = false;
    if (untilDate) {
      isSubscriptionValid = today <= untilDate;
    }
    
    console.log('📅 Subscription valid:', isSubscriptionValid);
    
    // Calculate token usage within the subscription period with error handling
    let isWithinTokenLimit = true;
    let tokensUsed = 0;
    const tokensAllowed = userData.tokens_allowed || 0;
    
    if (userData.start_date && userData.until_date) {
      console.log('🔍 Querying token usage with params:', {
        userEmail,
        start_date: userData.start_date,
        until_date: userData.until_date
      });
      ...
      try {
        // Get sum of token usage for the user within the subscription period
        const { data: tokenData, error: tokenError } = await supabase
          .from('pmc_user_tokens_usage')
          .select('token_usage')
          .eq('user_email', userEmail)
          .gte('usage_date', userData.start_date)
          .lte('usage_date', userData.until_date);
        
        if (tokenError) {
          console.error('❌ Error fetching token usage data:', tokenError);
          // SECURITY: If we can't fetch token data, DENY access
          console.log('🚫 Unable to fetch token data, DENYING access for security');
          return {
            hasAccess: false,
            message: "Access denied: your subscription has expired or you have consumed all your available tokens. Please update your plan."
          };
        } else if (tokenData) {
          console.log('📊 Raw token usage data:', tokenData);
          
          // Sum up all token usage
          tokensUsed = 0;
          tokensUsed = tokenData.reduce((sum, record) => {
            const usage = record.token_usage || 0;
            console.log(`  Adding ${usage} tokens`);
            return sum + usage;
          }, 0);
          ...
          console.log('🔢 Token calculation:', {
            tokensUsed,
            tokensAllowed,
            difference: tokensUsed - tokensAllowed,
            isWithinTokenLimit
          });
        }
      } catch (fetchError) {
        console.error('❌ Network error fetching token usage data:', fetchError);
        // If we can't fetch token data due to network issues, allow access as fallback
        console.log('🚨 Token data fetch failed, allowing access as fallback');
        isWithinTokenLimit = true;
        tokensUsed = 0;
      }
    } else {
      // If no subscription dates are set, allow access
      console.log('⚠️ No subscription dates set, allowing access');
      isSubscriptionValid = true;
      isWithinTokenLimit = true;
    }
    
    // User has access only if both subscription is valid AND within token limit
    const hasAccess = isSubscriptionValid && isWithinTokenLimit;
    ...
    console.log('🎯 FINAL ACCESS DECISION:', { 
      ...
      tokensAllowed,
      reason: !hasAccess ? (
        !isSubscriptionValid ? 'Subscription expired' : 
        !isWithinTokenLimit ? 'Token limit exceeded' : 'Unknown'
      ) : 'Access granted'
    });
    ...
    // SECURITY: In case of any error, DENY access for security
    console.log('🚫 Access check error, DENYING access for security');
    return {
      hasAccess: false,
      message: "Access denied: your subscription has expired or you have consumed all your available tokens. Please update your plan."
    };
...
// Function to get all users token usage (admin only)
export const adminGetAllTokenUsage = async () => {
  try {
    console.log('Fetching all users token usage via admin edge function');
    ...
    // Call the secure edge function to get all users token usage
    const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/admin-get-all-token-usage`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${session.access_token}`,
    ...
    const result = await response.json();
    console.log('All users token usage fetched successfully via edge function');
    ...
    console.error('Error in adminGetAllTokenUsage:', error);
    return {
      data: null,
      error
    };
...
export const adminGetUsers = async () => {
  try {
    console.log('Fetching all users via admin edge function');
    ...
    // Call the secure edge function to get all users
    ...
    const result = await response.json();
    console.log('Users fetched successfully via edge function');
...
export const adminUpdateUser = async (userData: {
  ...
  try {
    console.log('Updating user via edge function:', userData.userId);
    ...
    // Call the secure edge function for admin user update
...
export const adminDeleteUser = async (userId: string) => {
  try {
    ...
    // Call the secure edge function for admin user deletion
...
// Function to get beta registrations count (admin only)
export const adminGetBetaRegistrationsCount = async () => {
  try {
    console.log('Fetching beta registrations count via admin edge function');
    ...
    try {
      ...
      // Call the secure edge function to get beta registrations count
      ...
      const result = await response.json();
      console.log('Beta registrations count fetched successfully via edge function');
      ...
      console.error('Network error in adminGetBetaRegistrationsCount:', fetchError);
      // Return fallback data for network issues
      return {
        data: 0,
        error: new Error('Unable to connect to beta registration service. Please check your internet connection and Supabase configuration.')
      };
    }
  } catch (error) {
    console.error('Error in adminGetBetaRegistrationsCount:', error);
    return {
      data: 0,
      error
    };
